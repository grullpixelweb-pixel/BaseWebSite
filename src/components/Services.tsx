...
export default function Services() {
  ...
  return (
    <section
      id="servicios"
      className="py-24 bg-gray-900 border-t border-white/5 relative z-10"
    >
        ...
        <div className="mt-20 grid grid-cols-1 gap-8 md:grid-cols-2 lg:grid-cols-3">
          {services.map((service) => {
            ...
            return (
              <div
                key={service.id}
                className={`relative flex flex-col p-8 glass rounded-2xl shadow-xl transition-all duration-300 transform hover:-translate-y-2 hover:shadow-2xl hover:shadow-blue-500/20 ${service.id === 2 ? "border-2 border-violet-500" : ""}`}
              >
                {service.id === 2 && (
                ...
                <div className="flex-1">
                  <h3 className={`text-2xl md:text-3xl font-extrabold pb-1 tracking-tight text-transparent bg-clip-text bg-gradient-to-r 
                    ${service.id === 1 ? 'from-cyan-400 to-blue-500' : 
                      service.id === 2 ? 'from-violet-400 to-fuchsia-400' : 
